...
class Retriever:
    ...
    def search(self, question, category_filter="All"):
        log_secure_action("search", file_name="", count=3)
        embedding = generate_embedding(str(question))
        if self.store is None:
            return []
        return self.store.search(embedding, k=5, category_filter=category_filter)

    def index_document(self, file_path):
        file_path = str(file_path)
        filename = os.path.basename(file_path)
        category = map_filename_to_category(filename)
    ...
    def load_existing_index(self):
        ...
        if hasattr(self.store, 'documents') and self.store.documents:
            ...
            for doc in self.store.documents:
                if isinstance(doc, dict) and "filename" in doc:
                    unique_files.add(doc["filename"])
            self.indexed_files_list = unique_files if unique_files else {"Loaded Corporate Assets"}
