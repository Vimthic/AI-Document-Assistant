...
# 2. Configure the window viewport layout
st.set_page_config(
...
st.write("Ask questions about company operational protocols stored securely in the system.")

# Display persistent upload completion confirmation alerts safely
...
# Initialize the RAG engine once and preserve it across page refreshes
@st.cache_resource
...
try:
    ...
    st.error(f"Initialization error: Check your configs. Details: {e}")
    st.stop()
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        try:
            ...
            response_stream = st.session_state.rag_service.ask(
                str(question), 
                chat_history_logs=bounded_history
            )
            ...
            force_travel_only = "travel" in lower_question and "company" not in lower_question
            
            query_words = [w.lower().strip("?!.,") for w in question.split() if len(w) > 3 and w.lower() != "policy"]
            
            for source_item in matched_sources:
                if isinstance(source_item, dict) and "filename" in source_item:
